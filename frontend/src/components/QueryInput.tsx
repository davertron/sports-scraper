...
export function QueryInput({ transforms }: QueryInputProps) {
  ...
  function updatePentatonicOfRoot(transformIndex: number, value: string) {
    const newTransforms = [...transforms.value];
    newTransforms[transformIndex].args[1] = value;
    transforms.value = newTransforms;
  }

  function updatePentatonicOfQuality(transformIndex: number, value: string) {
    const newTransforms = [...transforms.value];
    newTransforms[transformIndex].args[2] = value;
    transforms.value = newTransforms;
  }
        ...
        <div>
          ...
          {t.type === 'filter' && (
            <select value={t.args[0]} onChange={e => updateFilter(transformIndex, (e.target as HTMLSelectElement).value)}>
              ...
              <option value="pentatonic-of">Pentatonic of</option>
            </select>
          ...
          {t.type === 'filter' && t.args[0] === 'pentatonic-of' && (
            <>
              <select value={t.args[1]} onChange={e => updatePentatonicOfRoot(transformIndex, (e.target as HTMLSelectElement).value)}>
                {NOTE_ORDER.map(note => (
                  <option value={note}>{note}</option>
                ))}
              </select>
              <select value={t.args[2]} onChange={e => updatePentatonicOfQuality(transformIndex, (e.target as HTMLSelectElement).value)}>
                <option value="major">Major</option>
                <option value="minor">Minor</option>
              </select>
            </>
